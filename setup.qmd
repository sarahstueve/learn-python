...
To install `uv`, we will run a command in a new `Terminal` window in VSCode using the `Terminal` tab in the top left corner of your screen or VSCode window. Once you have the `Terminal` open, run the following command or look at the documentation for alternatives [here](https://docs.astral.sh/uv/getting-started/installation/):
...
Alternatively, you can install it using Python, but you'll have to add `python -m` before the `uv` commands you run below.

If you're on a Mac, you may have to use `pip3` instead of `pip`.
```{bash}
pip install uv
```
...
Finally, we run the command `python -m uv sync` to make sure our environment and the project file created by uv (`pyproject.toml`) are synced together.
